...
bool sendInfo()   //Wasser Temperatur und Licht muss ausgelesen und gesendet werden
{
  ...
  StrWasser.toCharArray(chWasser,StrWasser.length()+1);
  StrLicht.toCharArray(chLicht,StrLicht.length()+1);
  StrTemperatur.toCharArray(chTemperatur,StrTemperatur.length()+1);
...
int LDR() {
  ...
  */
  ...
  double Prozent = Eingang/(40.96f);

  //Prozent = (Prozent * -1) + 100;         //Invertierung
